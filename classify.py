...
class SentenceClassification(CsvDataset):
    labels = ("0", "1")
    def __init__(self, file, pipline=[]):
        super().__init__(file, pipline)

    def get_instances(self, lines):
        for line in itertools.islice(lines, 1, None): # skip header
            yield line[-1], line[0], False


class PPClassification(CsvDataset):
    ...
    def get_instances(self, lines):
        for line in itertools.islice(lines, 1, None): # skip header
            yield line[1], line[2], False

def dataset_class(task):
    """ Mapping from task string to Dataset Class """

    table = {'mrpc': MRPC, 'mnli': MNLI, 'SST-2': SentenceClassification, 'pp': PPClassification}
    return table[task]
...
def main(task='pp',
         ...
         data_file='glue_data/pp/test.tsv',
         model_file='save/model_steps_28368.pt',
    ...
    if mode == 'train':
        ...
        resultwriter = open("./save/predict_result.tsv", 'w', encoding="utf-8")
        ...
        def evaluate(model, batch):
            ...
            for i in range(len(label_id)):
                resultwriter.writelines(str(int(label_pred[i])) + "\t" + str(int(label_id[i])) + "\t" + "none" + "\n")
